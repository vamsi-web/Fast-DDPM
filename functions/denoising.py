...
def pet_generalized_steps(x, x_pet, seq, model, b, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            # PET modification: concatenate PET-related information
            et = model(torch.cat([x_pet, xt], dim=1), t)
...
def pet_ddpm_steps(x, x_pet, seq, model, b, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        xs = [x]
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            # PET modification: concatenate PET-related information
            output = model(torch.cat([x_pet, x], dim=1), t.float())
            e = output
